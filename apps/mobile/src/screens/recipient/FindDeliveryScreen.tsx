...
type Props = NativeStackScreenProps<RecipientStackParamList, 'FindDelivery'>;

export function FindDeliveryScreen({ navigation }: Props) {
  ...
  const handleLookup = useCallback(async () => {
    ...
    try {
      ...
      // restriction, so this works even before a PIN verification is attempted.
      await consignmentsApi.getConsignmentById(consignmentId.trim());
      navigation.navigate('RecipientConsignmentDetail', { consignmentId: consignmentId.trim() });
    } catch (error) {
